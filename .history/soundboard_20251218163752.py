...
<body>
    ...
    <div id="nosounds" class="no-sounds" style="display:none;">
        No sound files found in the <b>sounds</b> folder.<br>
        Put some .mp3, .wav or .ogg files in <code>sounds/</code> and restart.
    </div>
    ...
    <script>
        ...
        async function refreshSounds() {{
            try {{
                const sounds = await pywebview.api.get_sounds();
                // Signature based on the ordered set of file paths (Python already sorts by name)
                const signature = JSON.stringify((sounds || []).map(s => s.path));
                if (signature === lastSignature) return;
                lastSignature = signature;
                renderSounds(sounds);
            }} catch (e) {{
                // If the API isn't ready yet, just wait for the next tick
                console.warn('refreshSounds failed', e);
            }}
        }}

        window.addEventListener('pywebviewready', function() {{
            refreshSounds();
            // Poll for file changes so newly added MP3s get buttons without restarting.
            setInterval(refreshSounds, 1500);
        }});
    </script>
</body>
</html>
