...
# App branding assets expected in the project/app folder.
FAVICON_ICO = BASE_DIR / 'favicon.ico'
...
<head>
    <meta charset="utf-8">
    <link rel="icon" href="{FAVICON_ICO.as_uri() if FAVICON_ICO.exists() else ''}">
...
def main():
    api = Api()
    # Prefer a native window icon when supported by the current pywebview backend.
    # pywebview expects a path to a PNG/ICO; we'll use favicon.ico if present.
    icon_path = None
    try:
        if FAVICON_ICO.exists():
            icon_path = str(FAVICON_ICO)
        elif (BASE_DIR / 'icon.png').exists():
            icon_path = str(BASE_DIR / 'icon.png')
        elif LOGO_PNG.exists():
            icon_path = str(LOGO_PNG)
    except Exception:
        icon_path = None
